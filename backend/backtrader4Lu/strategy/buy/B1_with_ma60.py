...
class B1WithMA60BuyStrategy(BuyStrategy):

    strategy_name = "B1_with_ma60"
    ...
    7) 最近 lookback_n 内，存在一次最低价小于等于MA60的价格，但每天的收盘价要高于MA60
    """
    def check_signal(self):
        ...
        if len(self.strategy.dataclose) >= lookback_n:
            condition7_1 = any(
                self.strategy.datalow[-i] <= self.strategy.double_line.ma1[-i]
                for i in range(0, lookback_n)
            )
            condition7_2 = all(
                self.strategy.dataclose[-i] >= self.strategy.double_line.ma1[-i]
                for i in range(0, lookback_n)
            )
            condition7 = condition7_1 and condition7_2
        else:
            condition7 = False
        ...
        # if (self.datas[0].datetime.date(0) >= pd.to_datetime('2025-10-23').date()
        #         and self.datas[0].datetime.date(0) <= pd.to_datetime('2025-11-14').date()):
        #     self.log(
        #         f"指定日期输出 - "
        #         f"KDJ_J={kdj_j:.2f}(<=13:{condition1}), "
        #         f"收盘价={self.dataclose[0]:.2f}>多空线={self.double_line.duokong_line[0]:.2f}({condition2}), "
        #         f"condition3={condition3}, "
        #         f"condition4={condition4}, "
        #         f"trend_line={self.double_line.trend_line[0]:.2f}>duokong_line={self.double_line.duokong_line[0]:.2f}({condition6}), "
