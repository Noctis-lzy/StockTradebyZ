...
class Down20:
    ...
    def _calculate_short_term(self, df: pd.DataFrame) -> pd.Series:
        """计算短期指标: 100*(C-LLV(L,3))/(HHV(C,3)-LLV(L,3))"""
        llv_l = df['low'].rolling(window=3, min_periods=3).min()
        hhv_c = df['close'].rolling(window=3, min_periods=3).max()
        return 100 * (df['close'] - llv_l) / (hhv_c - llv_l)

    def _calculate_long_term(self, df: pd.DataFrame) -> pd.Series:
        """计算长期指标: 100*(C-LLV(L,21))/(HHV(C,21)-LLV(L,21))"""
        llv_l = df['low'].rolling(window=21, min_periods=21).min()
        hhv_c = df['close'].rolling(window=21, min_periods=21).max()
        return 100 * (df['close'] - llv_l) / (hhv_c - llv_l)
    ...
    def select(
        ...
        for code, df in data.items():
            ...
            if len(hist) < 21 + 2:  # 需要足够的数据计算长期指标（21天）+ 2天历史数据
                continue
            ...
            # 计算短期和长期指标
            hist['short_term'] = self._calculate_short_term(hist)
            hist['long_term'] = self._calculate_long_term(hist)

            # 获取最近3天的数据（date日、date-1日、date-2日）
            recent_data = hist.tail(3)
            if len(recent_data) < 3:
                continue

                # # 检查条件（按时间顺序：date-2日、date-1日、date日）
                # # date-2日：短期和长期都大于80
                # day_minus_2 = recent_data.iloc[0]
                # # date-1日：长期大于80，短期小于35
                # day_minus_1 = recent_data.iloc[1]
                # # date日：短期和长期都大于80
                # current_day = recent_data.iloc[2]
                #
                # # 检查所有条件是否满足（添加NaN值检查）
                # condition1 = (not pd.isna(day_minus_2['short_term']) and
                #               not pd.isna(day_minus_2['long_term']) and
                #               day_minus_2['short_term'] > 80 and
                #               day_minus_2['long_term'] > 80)
                #
                # condition2 = (not pd.isna(day_minus_1['short_term']) and
                #               not pd.isna(day_minus_1['long_term']) and
                #               day_minus_1['long_term'] > 77 and
                #               day_minus_1['short_term'] < 30)
                #
                # condition3 = (not pd.isna(current_day['short_term']) and
                #               not pd.isna(current_day['long_term']) and
                #               current_day['short_term'] > 80 and
                #               current_day['long_term'] > 80)
                #
            ...
            condition2 = (not pd.isna(day_minus_1['short_term']) and
                              not pd.isna(day_minus_1['long_term']) and
                              day_minus_1['long_term'] >= 80 and
                              day_minus_1['short_term'] >= 80)

            condition3 = (not pd.isna(current_day['short_term']) and
                              not pd.isna(current_day['long_term']) and
                              current_day['short_term'] <= 35 and
                              current_day['long_term'] >= 80)
...
class B2:
    ...
    # 设置为静态方法
    @staticmethod
    def checkIsB2(date: pd.Timestamp, df: pd.DataFrame) -> bool:
        """
        对单只股票进行B2策略筛选
        ...
        j_today = float(kdj["J"].iloc[-1])
        # 检查前十天中任意一天的J值小于13
        j_values_last_10_days = kdj["J"].iloc[-11:-1]  # 获取前十天的数据（不包括今天）
        j_yesterday_condition = (j_values_last_10_days < 13).any()  # 检查是否有任意一天小于13

        # 判断j值条件
        if not (j_today < 65 and j_yesterday_condition):
            return False

        # 放量条件
        if not (hist.iloc[-1]["volume"] > hist.iloc[-2]["volume"]):
            return False
        ...
        if not (price_change_pct > 4):  # 涨幅大于4%
            return False

        # 上影线小于2%
        today = hist.iloc[-1]
        upper_shadow_pct = (today['high'] - max(today['open'], today['close'])) / max(today['open'],
                                                                                      today['close']) * 100
        if not (upper_shadow_pct < 4):  # 上影线小于4%
            return False
...
class GlodAcross: 
    ...
    def select(self, date: pd.Timestamp, data: Dict[str, pd.DataFrame]) -> List[str]:
        ...
        for code, df in data.items():
            # 筛选日期小于等于指定日期的数据
            hist = df[df["date"] <= date].copy()
            if len(hist) < 36:  # 需要足够的数据计算 EMA(26) + EMA(9) + 一些缓冲
                continue
            ...
            # 检测金叉 (DIF 上穿 DEA)
            # CROSS(DIF, DEA) 表示前一天 DIF < DEA，今天 DIF >= DEA
            cross_up = (dif.shift(1) < dea.shift(1)) & (dif >= dea)
            
            # 检测死叉 (DEA 上穿 DIF)
            cross_down = (dea.shift(1) < dif.shift(1)) & (dea >= dif)

            # 找到所有金叉的位置
            cross_up_positions = cross_up[cross_up].index
            if len(cross_up_positions) < 2:  # 需要至少两次金叉
                continue

            # 当前金叉（最后一次金叉）
            current_cross_pos = cross_up_positions[-1]
            current_cross_idx = hist.index.get_loc(current_cross_pos)

            # 前一次金叉（倒数第二次金叉）
            prev_cross_pos = cross_up_positions[-2]
            prev_cross_idx = hist.index.get_loc(prev_cross_pos)

            # 计算前一次金叉到当前金叉的天数
            bars_since_prev_cross = current_cross_idx - prev_cross_idx

            # 检查条件：
            # 1. 当前金叉
            if not cross_up.iloc[-1]:
                continue

            # 2. 水上条件：当前 DIF > 0 AND DEA > 0
            current_dif = dif.iloc[-1]
            current_dea = dea.iloc[-1]
            if not (current_dif > 0 and current_dea > 0):
                continue

            # 3. 前一次金叉到当前金叉的天数 <= 30
            if bars_since_prev_cross > 30:
                continue

            # 4. 前一次金叉也在水上：前一次金叉时 DIF > 0 AND DEA > 0
            prev_dif_at_cross = dif.iloc[prev_cross_idx]
            prev_dea_at_cross = dea.iloc[prev_cross_idx]
            if not (prev_dif_at_cross > 0 and prev_dea_at_cross > 0):
                continue

            # 5. 中间有死叉：在前一次金叉到当前金叉之间存在死叉
            # 检查前一次金叉之后到当前金叉之前的时间段
            if prev_cross_idx + 1 < current_cross_idx:
                has_dead_cross = cross_down.iloc[prev_cross_idx+1:current_cross_idx].any()
...
class B3:
    ...
    def select(self, date: pd.Timestamp, data: Dict[str, pd.DataFrame]) -> List[str]:
        ...
        for code, df in data.items():
            # date的上一个日期是否为B2买点
            if B2.checkIsB2(date - pd.Timedelta(days=1), df):
                if B3.checkIsB3(date, df):
                    picks.append(code)

        return picks

    @staticmethod
    def checkIsB3(date: pd.Timestamp, df: pd.DataFrame) -> bool:
        """
        对单只股票进行B3策略筛选
        十字星阳线:= C>=O AND ABS(C-O)/O <= 0.025 AND C>=REF(C,1);
        十字星阴线:= C<O AND ABS(C-O)/O<0.025 AND VOL<=REF(VOL,1)*0.6;
        Parameters
        ----------
        date : pd.Timestamp
            目标日期
        ...
        previous_volume = yesterday['volume']
        # 判断十字星条件

        # 计算实体与开盘价的比例
        body_ratio = abs(current_close - current_open) / current_open

        # 十字星阳线: C>=O AND ABS(C-O)/O <= 0.025 AND C>=REF(C,1)
        is_bullish_doji = (current_close >= current_open and
                           body_ratio <= 0.025 and
                           current_close >= previous_close)

        # 十字星阴线: C<O AND ABS(C-O)/O<0.025 AND VOL<=REF(VOL,1)*0.6
        is_bearish_doji = (current_close < current_open and
                           body_ratio < 0.025 and
                           current_volume <= previous_volume * 0.6)
        # 满足十字星阳线或十字星阴线条件之一
        if not (is_bullish_doji or is_bearish_doji):
            return False
